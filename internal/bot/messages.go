...
const (
	msgWelcome = `Привет! Я бот поддержки <a href="https://t.me/takoevpn_bot">TakoeVPN</a>.

Если у тебя вопрос — нажми на FAQ.

Если остались вопросы - опиши проблему, а также не забудь указать:
- используемую ОС (android/ios/windows)
- приложить скриншот из happ (или того приложения которое вы используете)
- ваш регион
- не работает wifi или мобильный интернет

Обращения не содержащие полезной нагрузки будут ПРОИГНОРИРОВАНЫ.`
...
Если не получается — опиши проблему текстом, укажи устройство и клиент.`,
	...
	{
		Label: "📲 Подключить второе устройство?",
		Answer: `*Как подключить второе устройство?*

Используй ту же самую ссылку на подписку, которую получил при первом подключении.

Одна подписка = 3 устройства одновременно.`,
	},
	{
...
2. Оплати нужную сумму.
3. Подписка продлится автоматически.
4. Если этого не произошло — напиши нам в чат, разберёмся.
...
Все клиенты бесплатны, ищи в официальных магазинах или на GitHub.`,
	...
	{
		Label: "⏱️ Ошибка таймаута",
		Answer: `*Выскакивает ошибка таймаута*

Проверь ссылку на подписку — она должна начинаться с https://ssb.

Если ссылка отличается или просто попробуй добавить подписку заново:
1. В нашем боте @takoevpn_bot зайди в Мои подписки
2. Выбери свою подписку
3. Нажми Доступ
4. Скопируй новую ссылку и добавь подписку заново.`,
	},
	{
